...
Open the .ipynb file. Modify parameters such as thermal diffusivity ($\alpha$), length ($L$), or the initial condition function ($f(x)$) in the configuration cells to explore different physical behaviors.Run all cells to render the 3D surface plot.
